...
const options = ["Sangat", "Cukup", "Kurang", "Tidak"];
...
export default function CaveSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post("/public-survey/THE_CAVE", {
        data: {
          direction,
          variety,
          service,
      ...
      <div className="min-h-screen flex items-center justify-center bg-emerald-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow p-6 text-center space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Terima kasih atas saran dan kritik Anda untuk The Cave Cafe.
          </p>
        </div>
      </div>
    );
  ...
  const renderRadioRow = (label: string, value: string, setter: (v: string) => void) => (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <button
          ...
          className={`border rounded px-3 py-2 text-left ${value === options[0] ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
        >
          {options[0]} jelas
        </button>
        <button
          ...
          className={`border rounded px-3 py-2 text-left ${value === options[1] ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
        >
          {options[1]} jelas
        </button>
        <button
          type="button"
          onClick={() => setter(options[2])}
          className={`border rounded px-3 py-2 text-left ${value === options[2] ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
        >
          {options[2]} jelas
        </button>
        <button
          type="button"
          onClick={() => setter(options[3])}
          className={`border rounded px-3 py-2 text-left ${value === options[3] ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
        >
          {options[3]} jelas
        </button>
      </div>
      ...
      <div className="max-w-md mx-auto bg-white min-h-screen md:min-h-0 md:my-8 md:rounded-2xl md:shadow-xl overflow-hidden pb-12">
        <div className="bg-[#0F4D39] text-white p-6 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-green-100 text-sm mt-1">Guest Comment • The Cave Cafe</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input className="border rounded px-3 py-2 text-sm" placeholder="Nama Lengkap" value={name} onChange={(e)=>setName(e.target.value)} />
            <input className="border rounded px-3 py-2 text-sm" placeholder="Nomor WhatsApp" value={phone} onChange={(e)=>setPhone(e.target.value)} />
            <input className="border rounded px-3 py-2 text-sm md:col-span-2" placeholder="Email" value={email} onChange={(e)=>setEmail(e.target.value)} />
            <input className="border rounded px-3 py-2 text-sm md:col-span-2" placeholder="Alamat" value={address} onChange={(e)=>setAddress(e.target.value)} />
          </div>
          {renderRadioRow(
            "Bagaimana menurut Anda petunjuk ke The Cave Cafe?",
            direction,
            setDirection
          )}
          {renderRadioRow(
            "Bagaimana menurut Anda tentang variasi makanan dan minuman kami?",
            variety,
            setVariety
          )}
          {renderRadioRow(
            "Bagaimana menurut Anda pelayanan dan kecepatan kami melayani Anda?",
            service,
            setService
          )}
          {renderRadioRow(
            "Bagaimana menurut Anda rasa makanan dan minuman kami?",
            taste,
            setTaste
          )}
          {renderRadioRow(
            "Bagaimana menurut Anda kebersihan area makan?",
            cleanliness,
            setCleanliness
          )}
          ...
          <div className="space-y-2">
            <p className="font-medium text-sm text-gray-800">
              Tuliskan komentar Anda untuk kami:
            </p>
            <textarea
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="p-6 overflow-y-auto space-y-6 text-sm text-gray-700 leading-relaxed custom-scrollbar">
                  ...
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">1</span>Persetujuan Pengguna</h4>
                    <p className="pl-7">Kami akan meminta persetujuan Anda secara jelas dan sah sebelum memproses data pribadi...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">2</span>Keterbukaan Informasi</h4>
                    <div className="pl-7">
                  ...
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">3</span>Pemrosesan yang Terbatas dan Sah</h4>
                    <p className="pl-7">Data pribadi Anda akan diproses secara terbatas, spesifik, sah, dan transparan...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">4</span>Akurasi dan Keamanan Data</h4>
                    <p className="pl-7">Kami berupaya memastikan data pribadi yang kami kelola akurat, lengkap, dan terlindungi...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">5</span>Kerahasiaan Data</h4>
                    <p className="pl-7">Kerahasiaan data pribadi Anda dijaga dengan baik...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">6</span>Pengawasan Pihak Terkait</h4>
                    <p className="pl-7">Apabila terdapat pihak ketiga yang terlibat dalam pemrosesan data...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">7</span>Langkah Keamanan Teknis dan Operasional</h4>
                    <p className="pl-7">Kami menerapkan langkah-langkah teknis dan operasional yang wajar...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">8</span>Masa Penyimpanan Data</h4>
                    <p className="pl-7">Data pribadi disimpan selama diperlukan atau hingga Anda meminta penghapusan...</p>
                  </section>
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">9</span>Hak Pengguna</h4>
                    <div className="pl-7">
                  ...
                  <section>
                    <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2"><span className="bg-[#0F4D39] text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">10</span>Pemberitahuan Insiden Data</h4>
                    <p className="pl-7">Jika terjadi insiden yang berpotensi mengganggu perlindungan data pribadi, kami akan memberitahukan paling lambat 3x24 jam.</p>
                  </section>
                  <div className="mt-2 p-4 bg-gray-100 rounded-xl text-center">
                    ...
                    <p className="text-gray-600 text-sm mb-2">Untuk pertanyaan/pengaduan terkait data pribadi, hubungi Customer Care:</p>
                    <a href="https://wa.me/628112264808" target="_blank" className="inline-flex items-center gap-2 text-[#0F4D39] font-bold hover:underline">📞 0811 2264 808</a>
                  </div>
                </div>
