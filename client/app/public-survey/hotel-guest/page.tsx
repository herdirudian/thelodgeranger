...
const expOpts = ["Excellent", "Good", "Fair", "Poor"];

export default function HotelGuestSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post("/public-survey/HOTEL_GUEST", {
        data: {
          checkIn,
          cleanliness,
          housekeeping,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-emerald-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow p-6 text-center space-y-3">
          ...
          <p className="text-sm text-gray-600">Terima kasih atas penilaian Anda untuk penginapan.</p>
        </div>
      </div>
    );
  }

  const renderExpRow = (label: string, value: string, setter: (v: string) => void) => (
      ...
      <div className="max-w-md mx-auto bg-white min-h-screen md:min-h-0 md:my-8 md:rounded-2xl md:shadow-xl overflow-hidden pb-12">
        <div className="bg-[#0F4D39] text-white p-6 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-green-100 text-sm mt-1">Guest Comment Penginapan</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <input className="border rounded px-3 py-2 text-sm" placeholder="Phone" value={phone} onChange={(e)=>setPhone(e.target.value)} />
            <input className="border rounded px-3 py-2 text-sm" placeholder="Room" value={room} onChange={(e)=>setRoom(e.target.value)} />
            <input className="border rounded px-3 py-2 text-sm" placeholder="Date (YYYY-MM-DD)" value={date} onChange={(e)=>setDate(e.target.value)} />
          </div>

          <div className="space-y-2">
            <p className="font-medium text-sm text-gray-800">Tahu dari mana The Lodge Maribaya Camp & Village?</p>
            <div className="grid grid-cols-2 gap-2">
              {["Tiktok","Instagram","Google","Teman"].map(v => (
                <button
                  key={v}
                  type="button"
                  onClick={()=>toggleMulti(discoverFrom, setDiscoverFrom, v)}
                  className={`border rounded px-3 py-2 text-left ${discoverFrom.includes(v) ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
                >
                  {v}
                </button>
              ))}
          ...
          {renderExpRow("Check-in Experience", checkIn, setCheckIn)}
          {renderExpRow("Cleanliness", cleanliness, setCleanliness)}
          {renderExpRow("Housekeeping", housekeeping, setHousekeeping)}
          {renderExpRow("Staff Service", staffService, setStaffService)}
          {renderExpRow("Restaurant Food", restaurantFood, setRestaurantFood)}
          {renderExpRow("Amenities", amenities, setAmenities)}
          {renderExpRow("Overall Hotel Rating", overall, setOverall)}
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 space-y-3">
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>Saya ingin mendapatkan tindak lanjut atas keluhan atau masukan yang saya sampaikan.</span>
            </label>
            <label className="flex items-start gap-2 text-sm">
              <input type="checkbox" checked={privacyConsent} onChange={e=>setPrivacyConsent(e.target.checked)} />
              <span>
                Saya menyetujui pemrosesan data pribadi sesuai Kebijakan Privasi The Lodge Maribaya.
                <button type="button" onClick={()=>setShowPrivacyModal(true)} className="block text-[#0F4D39] font-semibold underline mt-1">Baca lebih lanjut</button>
              </span>
            </label>
            <label className="flex items-start gap-2 text-sm">
              <input type="checkbox" checked={marketingConsent} onChange={e=>setMarketingConsent(e.target.checked)} />
              <span>
                Saya bersedia menerima informasi promo dan layanan after-sales dari The Lodge Maribaya.
                <button type="button" onClick={()=>setShowMarketingModal(true)} className="block text-[#0F4D39] font-semibold underline mt-1">Baca lebih lanjut</button>
              </span>
            </label>
          </div>
          ...
          <div className="space-y-2">
            <p className="font-medium text-sm text-gray-800">Other Experience:</p>
            <div className="grid grid-cols-2 gap-2">
              {["Reureuh/istrihat","Ameung/healing"].map(v => (
                <button
                  key={v}
                  type="button"
                  onClick={()=>toggleMulti(otherExperience, setOtherExperience, v)}
                  className={`border rounded px-3 py-2 text-left ${otherExperience.includes(v) ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700"}`}
                >
                  {v}
                </button>
              ))}
          ...
          <div className="space-y-2">
            <p className="font-medium text-sm text-gray-800">Tuliskan komentar Anda untuk kami:</p>
            <textarea
