...
const optDir = ["Sangat jelas", "Jelas", "Kurang jelas", "Tidak jelas"];
...
export default function OmahBambooSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post("/public-survey/OMAH_BAMBOO", {
        data: {
          direction,
          variety,
          service,
      ...
      <div className="min-h-screen flex items-center justify-center bg-emerald-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow p-6 text-center space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Terima kasih atas saran dan kritik Anda untuk Omah Bamboo.
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-md mx-auto bg-white min-h-screen md:min-h-0 md:my-8 md:rounded-2xl md:shadow-xl overflow-hidden pb-12">
        <div className="bg-[#0F4D39] text-white p-6 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-green-100 text-sm mt-1">Guest Comment • Omah Bamboo</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {renderRow("Bagaimana petunjuk ke Omah Bamboo?", optDir, direction, setDirection)}
